...
ASAP ->
    ...
    preload 'https://cdnjs.cloudflare.com/ajax/libs/jquery-scrollTo/2.1.3/jquery.scrollTo.min.js', ->
        ...
        $(document).on 'click', '[data-scrollto]', ->
            $(window).scrollTo $(this).data('scrollto'), 500, interrupt: true, margin: true, offset: -50
    ...
    showCardWithSelector = ($sel, clicked) ->
        ...
        if window.innerWidth > 768
            ...
            if clicked
                $.when($scrollToReady).done -> $(window).scrollTo $el2show, 500, interrupt: true, margin: true, offset: -50
        $('.accomodation-only a').attr href: $sel.attr('data-acc-href')
